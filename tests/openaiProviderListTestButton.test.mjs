...
test('OpenAI 列表支持顺序测试全部 provider 并逐个回写最新 revision', () => {
  ...
  assert.match(aiProvidersPageSource, /const testableOpenAIProviders = openaiProviders\.filter\(/);
  assert.match(aiProvidersPageSource, /entry\.priority === -10 \|\| entry\.priority === -100/);
  assert.match(aiProvidersPageSource, /for \(const entry of testableOpenAIProviders\) \{/);
