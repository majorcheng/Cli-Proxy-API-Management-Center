...
test('请求日志中英文文案包含输出速率列表头', () => {
  assert.equal(zhLocale.monitor.logs.header_output_throughput, '输出速率');
  assert.equal(enLocale.monitor.logs.header_output_throughput, 'Output Throughput');
});
...
test('请求日志表格把成功率列替换为输出速率列并同步扩展列宽', () => {
  ...
  assert.match(monitorPageStyles, /th:nth-child\(10\), td:nth-child\(10\)\s*\{\s*width:\s*120px;\s*\}/);
});
