...
const OFFICIAL_MODEL_PRICES: Readonly<Record<string, ModelPrice>> = Object.freeze({
  ...
  'gpt-5.2': Object.freeze({ prompt: 1.75, completion: 14, cache: 0.175 }),
});
...
const cloneModelPriceMap = (prices: Record<string, ModelPrice>): Record<string, ModelPrice> =>
  Object.fromEntries(
    Object.entries(prices).map(([model, price]) => [
      model,
      { prompt: price.prompt, completion: price.completion, cache: price.cache },
    ])
  );

const normalizeModelPriceMap = (value: unknown): Record<string, ModelPrice> => {
  if (!isRecord(value)) {
    return {};
  }
...
const hasConfiguredModelPrice = (
  modelName: string | undefined,
  modelPrices: Record<string, ModelPrice>
): modelName is string => Boolean(modelName && modelPrices[modelName]);

const loadStoredModelPriceOverrides = (): Record<string, ModelPrice> => {
  try {
    ...
    return normalizeModelPriceMap(JSON.parse(raw));
  } catch {
    return {};
  }
};

const buildModelPriceOverrides = (
  ...
  Object.entries(normalized).forEach(([model, price]) => {
    const officialPrice = OFFICIAL_MODEL_PRICES[model];
    if (officialPrice && areModelPricesEqual(price, officialPrice)) {
      return;
    }
...
/**
 * 返回内置官方模型单价。
 * 使用拷贝是为了避免调用方意外修改默认常量。
 */
export function getOfficialModelPrices(): Record<string, ModelPrice> {
  return cloneModelPriceMap(OFFICIAL_MODEL_PRICES as Record<string, ModelPrice>);
}

/**
 * 将本地覆盖价格与官方默认价格合并成最终价格表。
 */
...
export function extractTotalTokens(detail: unknown): number {
  ...
  // live usage 样本已验证：
  // - reasoning_tokens 已包含在 output_tokens 内
  // - cached_tokens / cache_tokens 是 input_tokens 的子集
  // 因此 total_tokens 缺失时只能回退到 input + output，避免把子项重复累计。
...
/**
 * 计算最近 N 分钟的 RPM/TPM。
 * 允许调用方传入固定 nowMs，让监控卡牌和刷新快照使用同一个时间基准。
 */
...
/**
 * 汇总一组请求明细的计费覆盖情况。
 * - totalCost 只统计已配置单价的模型
 * - 未配置单价的模型只计入 unpricedRequestCount / unpricedModels
 */
export function summarizePricingFromDetails(
  ...
  usageDetails.forEach((detail) => {
    ...
    if (!hasConfiguredModelPrice(modelName, modelPrices)) {
  ...
  return {
    ...
    pricedModels: Array.from(pricedModels).sort((a, b) => a.localeCompare(b)),
    unpricedModels: Array.from(unpricedModels).sort((a, b) => a.localeCompare(b)),
  };
}

/**
 * 从完整 usage 数据中汇总计费覆盖情况。
 */
export function summarizeUsagePricing(
  usageData: unknown,
  modelPrices: Record<string, ModelPrice>
): PricingCoverageSummary {
  return summarizePricingFromDetails(collectUsageDetails(usageData), modelPrices);
}
...
export function loadModelPrices(): Record<string, ModelPrice> {
  return resolveModelPrices(loadStoredModelPriceOverrides());
}

/**
 * 保存模型价格覆盖到 localStorage。
 * 默认官方价格不重复落盘，只持久化与默认值不同的覆盖项。
 */
export function saveModelPrices(prices: Record<string, ModelPrice>): void {
  try {
    ...
    localStorage.setItem(MODEL_PRICE_STORAGE_KEY, JSON.stringify(overrides));
  } catch {
